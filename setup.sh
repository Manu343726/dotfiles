...
optparse.define short=t long=target-confirm desc="Asks before installing a target, so you can select which targets to install while running" variable=target_confirm value=true default=false
optparse.define short=c long=component-confirm desc="Same as 'target-confirm', but for components. Implies low verbosity" variable=component_confirm value=true default=false
...
if $component_confirm && [ "$verbosity" == "$DOTFILES_VERBOSE_DISABLED" ]; then
    ...
    echo --target-confirm=${target_confirm}
    echo --component-confirm=${component_confirm}
